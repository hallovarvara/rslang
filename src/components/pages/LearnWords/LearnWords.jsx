...
export default class LearnWords extends Component {
  ...
  handleNextWord = () => {
    ...
    this.setState((state) => ({
      wordCount:
        state.wordCount === state.words.length - 1 ? 0 : state.wordCount + 1,
    }));
  };
  ...
  updateUserWordInState = (wordObject) => {
    this.setState((state) => ({
      words: state.words.map((el) =>
        el.id === wordObject?.id ? wordObject : el,
      ),
    }));
  ...
  handleStartNewLearning = async () => {
    ...
    const group = isWordsRandomly
      ? getRandomNumber(0, count.groups)
      : userLevel;
      ...
      this.setState((state) => ({
        ...
        userSettings: {
          isShownComplicatedButton:
            userSettings.optional.option.complicatedButton,
          isShownAnswerButton: userSettings.optional.option.showAnswerButton,
          isShownImageAssociation:
            userSettings.optional.option.imageAssociatation,
  ...
  render() {
    ...
    const { categoriesSelect } = settings;
