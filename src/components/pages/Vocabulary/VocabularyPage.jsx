...
class VocabularyView extends React.Component {
  ...
  componentDidMount() {
    this.vocabularyWords = JSON.parse(
      localStorage.getItem(localThings.WORDS) || '[]',
    );
    setTimeout(() => {
      this.setState({
        loading: false,
  ...
  render() {
    const { loading, filteredWords, currentPage } = this.state;
    ...
    const cards = filteredWords
      .slice(
        amountOfWordsPerPage * (currentPage - 1),
        currentPage * amountOfWordsPerPage,
      )
    ...
    return (
      <div className="vocabulary-page vocabulary-page-container">
        <div className="vocabulary-settings-container">
          <h1 className="vocabulary-settings-container__page-title">
            {pagesData.vocabulary.title}
          </h1>
          <Select
            className="vocabulary-selector"
            onChange={(event) => {
              const newSelectedOption = event.target.value;
              const newFilterPath = selectorOptions.find(
                (option) =>
                  option.title.toLowerCase() ===
                  newSelectedOption.toLowerCase(),
              ).fieldPath;
